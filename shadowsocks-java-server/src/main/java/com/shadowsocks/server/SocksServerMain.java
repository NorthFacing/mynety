...
import com.shadowsocks.server.Config.ConfigXmlLoader;
...
public class SocksServerMain {
	...
	public static void main(String[] args) throws Exception {
		...
		ConfigXmlLoader.loadServer("conf/server-config.xml");
		...
		try {
			...
			b.bind(ServerConfig.config.getLocalPort())
				.sync().channel().closeFuture().sync();         // sync：请求进行阻塞处理
		} finally {
