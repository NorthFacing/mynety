package com.adolphor.mynety.client.http.http_1_0;
 ...
 */
@Slf4j
public class Http_1_0_ConnectionHandler extends AbstractInRelayHandler<Object> {

  private DefaultHttpRequest httpRequest;

  public Http_1_0_ConnectionHandler(DefaultHttpRequest httpRequest) {
  ...
  public void channelActive(ChannelHandlerContext ctx) throws Exception {
    ...
    try {
      ...
      logger.error("[ " + clientChannel + LOG_MSG + connHost + ":" + connPort + " ] http1.0 connect internet error", e);
      channelClose(ctx);
    }
  ...
  protected void channelRead0(ChannelHandlerContext ctx, Object msg) throws Exception {
    logger.debug("[ {}{}{} ] http1.0 to socks handler receive http msg: {}", ctx.channel(), LOG_MSG, remoteChannelRef.get(), msg);
    Channel remoteChannel = remoteChannelRef.get();
    synchronized (requestTempLists) {
      if (isConnected) {
        ReferenceCountUtil.retain(msg);
        remoteChannel.writeAndFlush(msg);
        logger.debug("[ {}{}{} ] transfer http1.0 request to socks: {}", ctx.channel(), LOG_MSG_OUT, remoteChannel, msg);
      } else {
        if (msg instanceof HttpObject) {
          ReferenceCountUtil.retain(msg);
          requestTempLists.add(msg);
          logger.debug("[ {}{}{} ] add transfer http1.0 request to temp list: {}", ctx.channel(), LOG_MSG, remoteChannel, msg);
        } else {
          logger.warn("[ {}{}{} ] http1.0 unhandled msg type: {}", ctx.channel(), LOG_MSG, remoteChannel, msg);
        }
      }
    }
  }
