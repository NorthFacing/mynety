...
public class Socks03ConnectHandler extends SimpleChannelInboundHandler {
	...
	public void channelActive(ChannelHandlerContext ctx) {
		logger.info(Constants.LOG_MSG + ctx.channel() + "连接处理器激活，发送网页请求：" + ByteBufUtil.hexDump(buf));
	...
	protected void channelRead0(ChannelHandlerContext ctx, Object msg) throws Exception {
		...
		logger.info(Constants.LOG_MSG + ctx.channel() + "连接处理器收到消息：ver={}, cmd={}, psv={}, atyp={}, dstLen={}, addr={}, port={}",
	...
	public void channelReadComplete(ChannelHandlerContext ctx) throws Exception {
		...
		logger.info(Constants.LOG_MSG + ctx.channel() + "连接处理器任务完成，移除此处理器完毕");
		ctx.pipeline().addLast(new Socks04DataHandler());
		logger.info(Constants.LOG_MSG + ctx.channel() + "连接处理器任务完成，添加数据处理器");
		ctx.pipeline().fireChannelActive();
	}

	@Override
	public void exceptionCaught(ChannelHandlerContext ctx, Throwable throwable) {
		logger.error(Constants.LOG_MSG + ctx.channel() + "连接处理器异常：", throwable);
		ctx.channel().close();
	}

	@Override
	public void channelInactive(ChannelHandlerContext ctx) throws Exception {
		logger.info(Constants.LOG_MSG + ctx.channel() + "连接处理器连接断开：" + ctx.channel());
